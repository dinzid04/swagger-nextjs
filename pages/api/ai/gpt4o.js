...
async function processGPT4o(text, systemPrompt, imageUrl, sessionId = 'neko') {
  const params = {
  ...
  const apiUrl = `https://api.nekolabs.my.id/ai/gpt/4o?text=${params.text}&systemPrompt=${params.systemPrompt}&imageUrl=${params.imageUrl}&sessionId=${params.sessionId}`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (!text || !imageUrl) {
        return res.status(400).json({
          status: false,
          message: 'Parameters text and imageUrl are required'
        });
      }

      console.log('Processing GPT-4o with URL image...');
    ...
    // POST method - dengan upload file
      ...
      // Process dengan GPT-4o
      const gptResult = await processGPT4o(text, systemPrompt, cdnResult.url, sessionId);
